            ...
            <g key={`points-group-${i}`}>
              ...
              <g key={`start-${i}`}>
                ...
                {showLabels && dot.start.label && (
                  ...
                  >
                    <foreignObject
                      x={startPoint.x - 100 + (dot.start.labelOffset?.x || 0)}
                      y={startPoint.y - 35 + (dot.start.labelOffset?.y || 0)}
                      width="200"
                      height="40"
                      className="block overflow-visible"
                    >
                      <div className="flex items-center justify-center h-full">
                        <span className="text-sm font-medium px-2.5 py-1 rounded-md bg-white/95 text-slate-900 border border-orange-200 shadow-sm whitespace-nowrap">
                          {dot.start.label}
                        </span>
                      </div>
                    </foreignObject>
              ...
              <g key={`end-${i}`}>
                ...
                {showLabels && dot.end.label && (
                  ...
                  >
                    <foreignObject
                      x={endPoint.x - 100 + (dot.end.labelOffset?.x || 0)}
      ...
      <AnimatePresence>
        {hoveredLocation && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 10 }}
            className="absolute bottom-4 left-4 bg-white/90 text-slate-900 px-3 py-2 rounded-lg text-sm font-medium backdrop-blur-sm sm:hidden border border-orange-200"
          >
            {hoveredLocation}
          </motion.div>
        )}
